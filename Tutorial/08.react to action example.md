...
适用于可以累计的场景，使用 observable manage state 的关键操作符是 scan
...
retain 一个累计值（accumulator），用给定的 function 计算 total 值。total 可以是值的和，或一个可以添加或删除数据的数组。
...
merge 是静态的 combination creation function，不可在 pipe 中使用
...
合并具有相似类型的序列以混合他们的 emit items 到一个 output observable
